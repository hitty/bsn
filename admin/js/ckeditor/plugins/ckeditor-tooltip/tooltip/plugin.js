...
CKEDITOR.plugins.add('tooltip', {
  ...
  afterInit: function(editor) {
    CKEDITOR.addCss('.cke_editable .' + editor.config.tooltip_class + '{' + 'cursor:pointer;color:#1e88e5; border-bottom: 1px dashed #1e88e5;' + '}');
  },
...
CKEDITOR.removeTooltipCommand.prototype = {
  ...
  refresh: function(editor, path) {
    ...
    if (element && element.getName() == editor.config.tooltip_tag && element.getAttribute('data-title') && element.getChildCount())
...
CKEDITOR.config.tooltip_class = 'tooltip-data';
CKEDITOR.config.tooltip_html = true;
CKEDITOR.tooltip_toolbar = [
	{ name: 'document', groups: [ 'mode', 'document', 'doctools' ], items: [ 'Source' ] },

];
CKEDITOR.config.tooltip_styleSet = 'Small';
