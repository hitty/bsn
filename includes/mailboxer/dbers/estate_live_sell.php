...
function dber_estate_live_sell_remove($user_id, $remove_options)
{
	global $db;

	if($remove_options == REMOVE_ROBOT_ONLY)
	{
		$sql = "DELETE from flatdata.livtrad where user_id = '" . $user_id . "' AND arend = 'N' AND info_added_from = 'bsnrobot';";
	}
...
function dber_estate_live_sell($item_node)
{
	global $db;
	...
	// cost type paid (= NULL since we are doing live SELL)
		$dsql->add_text_part('by_the_day',NULL);
