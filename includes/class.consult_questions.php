...
class ConsultQuestion {
    ...
    */
    function __construct($id,$create_params = null,ConsultQuestion $parent_app = null,$no_notify = null){
    ...
    */
    private function Create($parameters){
        ...
        if(!empty($user_email) && Validate::isEmail($user_email) && !empty($user_name) && !empty($q_cat) && !empty($q_body) ){
            ...
            $this->data_array['visible_to_all'] = ((empty($this->id_respondent_user) || ($advanced_item == 2 && !$payed_page)) || ( !empty($respondent_user_info['id_agency']) && empty($agency_info['agency_tarif']) ) ? 1 : 2);
            ...
            $this->data_array = $db->fetch("SELECT ".$this->tables['consults'].".*,
                                            ...
                                            WHERE ".$this->tables['consults'].".id = ".$this->id);
    ...
    */
    public function sendAskedUserNotification($id_answer = false, $delayed_answer = false){
        ...
        // параметры письма
        $mailer = new EMailer('mail');
        if(empty($id_answer)) $mailer->Subject = iconv('UTF-8', $mailer->CharSet, 'Ваш вопрос ID '.$this->id.' в сервисе Консультант '.Host::$host.' прошел модерацию');
        else $mailer->Subject = iconv('UTF-8', $mailer->CharSet, 'На Ваш вопрос ID '.$this->id.' в сервисе Консультант '.Host::$host.' пришел ответ');
        $mailer->Body = $html;
        $mailer->AltBody = strip_tags($html);
        $mailer->IsHTML(true);
    ...
    */
    private function sendModeratorNotification($id_answer = false){
        ...
        if(!empty($notify_info['email'])){
            ...
            Response::SetBoolean('agent',(!empty($this->id_respondent_user) && $this->respondents_array['user_tarif']>0) );
    ...
    */
    ...
    public function refreshAnswersAmount(){
        ...
        $res = $db->querys("UPDATE ".$this->tables['consults']." SET answers_amount = ? WHERE id = ?",$answers_amount,$this->id);
        return $res;
    }
    ...
    */
    public function getAnswersList($all_statuses = false,$sortby = false){
        global $db;
        global $auth;
        ...
        $res = $db->fetchall("SELECT ".$this->tables['consults_answers'].".*,
                                     ...
                                     END AS status_title,
                                     IF(".$this->tables['consults_answers'].".id = ".$this->data_array['id_best_answer'].",1,0) AS is_best_answer,
                                     ".$this->tables['users'].".name AS user_name,
                              ...
                              FROM ".$this->tables['consults_answers']."
                              LEFT JOIN (SELECT id_parent,(SUM(vote_for)-SUM(vote_against)) AS rating, ".(!empty($auth->id)?"SUM(id_user = ".$auth->id.") AS voted":"0 AS voted")." FROM ".$this->tables['consults_answers_votings']." GROUP BY id_parent) v ON v.id_parent = ".$this->tables['consults_answers'].".id
                              LEFT JOIN ".$this->tables['users']." ON ".$this->tables['consults_answers'].".id_user = ".$this->tables['users'].".id
                              LEFT JOIN ".$this->tables['users_photos']." ON ".$this->tables['users_photos'].".id_parent=".$this->tables['users'].".id
                              LEFT JOIN ".$this->tables['agencies']." ON ".$this->tables['users'].".id_agency = ".$this->tables['agencies'].".id
                              WHERE ".$this->tables['consults_answers'].".id_parent = ?".(empty($all_statuses)?" AND ".$this->tables['consults_answers'].".status = 1 ":"")." 
                              ORDER BY ".$this->tables['consults_answers'].".id = ".$this->data_array['id_best_answer']." DESC".$sortby,false,$this->id);
        return $res;
    }
    ...
    */
    ...
    public function updateFromMapping($mapping){
        foreach($mapping as $key=>$item){
            if(!empty($this->data_array[$key])) $this->data_array[$key] = $mapping[$key]['value'];
        }
    }
...
abstract class ConsultQFunctions{
    ...
    */
    ...
    public static function validateAddFormParams($params){
        ...
        if(!empty($params['responder']) && !ConsultQFunctions::checkIfCanAnswer($params['responder'])) $errors['responder'] = 'Этот специалист не может ответить';
        elseif(!empty($field['mail']) && !Validate::isEmail($params['email'])) $errors['email'] = 'ошибка, неверный email';
        return $errors;
    }
    ...
    */
    public static function checkIfCanAnswer($id_user){
        ...
        //если пользователя нет, или это не юрист, или нет нужного тарифа
        return (!(empty($user_info) || $user_info['user_activity'] != 2 || empty($user_info['id_tarif']) || $user_info['id_tarif'] != 4));
    }
