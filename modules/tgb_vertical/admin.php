...
switch($action){
	...
	case 'archive':
		...
		if($id>0){
			$res = $db->query("UPDATE ".$sys_tables['tgb_vertical']." SET `published` = ? WHERE id=?", $status, $id) or die($db->error);
    ...
    case 'setStatus':
        ...
        if($id>0){
            $res = $db->query("UPDATE ".$sys_tables['tgb_vertical']." SET `enabled` = ? WHERE id=?", $status, $id);
