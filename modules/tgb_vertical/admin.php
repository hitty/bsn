<?php
$GLOBALS['js_set'][] = '/modules/tgb_vertical/ajax_actions.js';
...
// добавление title
$this_page->manageMetadata(array('title'=>'Вертикальный баннер'));
...
switch($action){
    ...
    case 'stats':
		...
		$GLOBALS['js_set'][] = '/modules/tgb_vertical/datepick_actions.js';
		//получение данных по объекту из базы
		$info = $db->fetch("SELECT 
								`id`,
								`title`,
								IF(img_link='',
									  CONCAT_WS('/','".Config::$values['img_folders']['tgb_vertical']."',`img_src`),
								`img_link`) as photo
							FROM ".$sys_tables['tgb_vertical']."
							WHERE `id` = ?",$id);
		...
		if(!empty($post_parameters['submit'])){
			...
			$stats = $db->fetchall("
					...
					(
					  ...
					  FROM ".$sys_tables['tgb_vertical_stats_full_shows']."
					  WHERE
					...
					(
					  SELECT 
						  ...
						  DATE_FORMAT(`date`,'%d.%m.%Y') as date
					  FROM ".$sys_tables['tgb_vertical_stats_full_clicks']."
					  WHERE
						  `date` >= STR_TO_DATE('".$date_start."', '%d.%m.%Y') AND 
						  `date` <= STR_TO_DATE('".$date_end."', '%d.%m.%Y') AND `id_parent` = ".$id."
					  GROUP BY `date`
	...
	case 'edit':
		...
		if($action=='add'){
			...
			$info = $db->prepareNewRecord($sys_tables['tgb_vertical']);
		} else {
		...
		Response::SetString('img_folder', Config::$values['img_folders']['tgb_vertical']); // папка для Вертикальный баннер
		...
		if(!empty($post_parameters['submit'])){
				...
				foreach ($_FILES as $fname => $data){
					if ($data['error']==0) {
                        ...
                        if($size[0]!=40 && $size[1]!=500) $mapping['banners']['img_src']['error'] = 'Размер файла должен быть 40x500px. Размер вашего файла'.$size[0].'x'.$size[1].'px';
                            ...
                            $_folder = Host::$root_path.'/'.Config::$values['img_folders']['tgb_vertical'].'/'; // папка для файлов  Вертикальный баннер
			...
			if(empty($errors)) {
				...
				if($action=='edit'){
					...
					$res = $db->updateFromArray($sys_tables['tgb_vertical'], $info, 'id') or die($db->error);
				} else {
					//дата дообавления объекта
					$res = $db->insertFromArray($sys_tables['tgb_vertical'], $info, 'id');
					if(!empty($res)){
						$new_id = $db->insert_id;
						// редирект на редактирование свеженькой страницы
						if(!empty($res)) {
							header('Location: '.Host::getWebPath('/admin/advert_objects/tgb_vertical/edit/'.$new_id.'/'));
							exit(0);
						}
					}
				}
	...
	case 'archive':
		...
		if($id>0){
			$res = $db->querys("UPDATE ".$sys_tables['tgb_vertical']." SET `published` = ? WHERE id=?", $status, $id) or die($db->error);
    ...
    case 'setStatus':
        ...
        if($id>0){
            $res = $db->querys("UPDATE ".$sys_tables['tgb_vertical']." SET `enabled` = ? WHERE id=?", $status, $id);
	...
	default:
		...
		//кол-во эл-ов в каждом блоке размещения
		 $sql = "
			SELECT  
				(SELECT IFNULL(COUNT(*),0) FROM ".$sys_tables['tgb_vertical']." WHERE `published` !=3 ) AS alls,
				(SELECT IFNULL(COUNT(*),0) FROM ".$sys_tables['tgb_vertical']." WHERE `published` = 1 and enabled = 1) AS active,
				(SELECT IFNULL(COUNT(*),0) FROM ".$sys_tables['tgb_vertical']." WHERE `published` = 3) AS archive
			FROM dual";
		...
		$paginator = new Paginator($sys_tables['tgb_vertical'], 30, $condition);
		...
		// ссылка пагинатора
		$paginator->link_prefix = '/admin/advert_objects/tgb_vertical'                // модуль
		...
		$sql = "SELECT 
                        ban.*, 
						IF(ban.img_link='',
							  CONCAT_WS('/','".Config::$values['img_folders']['tgb_vertical']."',`ban`.`img_src`),
				...
				FROM ".$sys_tables['tgb_vertical']." ban
		        LEFT JOIN (SELECT COUNT(*) as cnt_day, id_parent FROM ".$sys_tables['tgb_vertical_stats_day_shows']." GROUP BY id_parent) a ON a.id_parent = ban.id	
		        LEFT JOIN (SELECT SUM(amount) as cnt_full, id_parent FROM ".$sys_tables['tgb_vertical_stats_full_shows']." GROUP BY id_parent) b ON b.id_parent = ban.id	
		        LEFT JOIN (SELECT COUNT(*) as cnt_click_day, id_parent FROM ".$sys_tables['tgb_vertical_stats_day_clicks']." GROUP BY id_parent) c ON c.id_parent = ban.id		
                LEFT JOIN (SELECT SUM(amount) as cnt_click_full, id_parent FROM ".$sys_tables['tgb_vertical_stats_full_clicks']." GROUP BY id_parent) d ON d.id_parent = ban.id       
		        ";		
