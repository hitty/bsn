...
switch(true){
    ...
    case $action == 'left':
        if($ajax_mode){
            ...
            if(!empty($item)){
                $ajax_result['ok'] = true;
                if(!Host::$is_bot) $db->querys("INSERT INTO ".$sys_tables['tgb_vertical_stats_day_shows']." SET id_parent = ?", $item['id']);
    ...
    case $action=='click': // запись статистики клика
        if($ajax_mode){
            ...
            if($id>0){
                if(!Host::$is_bot) $res=$db->querys("INSERT INTO ".$sys_tables['tgb_vertical_stats_day_clicks']." SET `id_parent`=".$id);
                $ajax_result['ok'] = $res;
            }
        } else $this_page->http_code=404;
        break;
