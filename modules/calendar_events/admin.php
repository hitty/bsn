...
switch($action){
			...
			if(empty($errors)) {
				...
				if($action=='edit'){
					...
					if(!empty($res)){
                        ...
                        $db->query( "UPDATE ".$sys_tables['calendar_events']." SET `chpu_title` = ? WHERE `id` = ?", $new_id.'_'.createCHPUTitle($info['title']), $new_id);
                        //Матвей:end                        
	...
	case 'del':
        ...
        if($id>0){
		    ...
		    $res = $db->query("DELETE FROM ".$sys_tables['calendar_events']." WHERE id=?", $id);
