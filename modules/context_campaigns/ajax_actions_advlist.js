jQuery(document).ready(function(){
    var _now = new Date();
    _geopopup_handlers_included = false;
    
    //нвоый блок в список объявлений
    function addNewBlock(msg){
        var _initial_html = "<div id='context-block-"+msg.id+"' class='context-adv'><div class='left-block-box'><div class='img-preview'></div><div class='left-block unactive'></div></div><span class='object-actions'></span><div class='context-adv-edit'></div></div>";
        if(jQuery('.adv-list').children('.context-adv').length == 0) jQuery('.adv-list').append(_initial_html);
        else jQuery('.adv-list').children('.context-adv').last().after(_initial_html);
         
        //формируем блок с картинкой
        jQuery('#context-block-'+msg.id).children('.img-preview').html('<div class="campaign-img-box"><div class="campaign-img-frame"><img src=""></div></div>');
        //формируем блок с информацией
        jQuery('#context-block-'+msg.id).children('.left-block-box').children('.left-block').html(jQuery('#context-block-'+msg.id).children('.left-block-box').children('.left-block').html()+'<span class="info-panel"></span><span class="targeting-panel"></span><span class="stats-panel"></span>');
        var _context_block_info = jQuery('#context-block-'+msg.id).children('.left-block-box').children('.left-block').children('.info-panel');
        var _context_block_targeting = jQuery('#context-block-'+msg.id).children('.left-block-box').children('.left-block').children('.targeting-panel');
        var _context_block_stats = jQuery('#context-block-'+msg.id).children('.left-block-box').children('.left-block').children('.stats-panel');
        ...
        //собираем верхнюю строчку (название, картинка)
        _context_block_info.html('<span class="adv id"></span><span class="adv title"></span><span class="image-info">нет изображения</span>');
        _context_block_info.children('.adv.id').html("#" + msg.id);
        _context_block_info.children('.adv.title').html(msg.form_data['title']);
        //собираем среднюю строчку (таргетинг)
        _context_block_targeting.html("<span class='targeting-info'></span><span class='targeting-estate'></span>");
        _context_block_targeting.children(".targeting-info").html("<i class='title-text'>Таргетинг</i>");
        //собираем нижнюю строчку (статистика)
        _context_block_stats.html('<span class="stats-info"><span class="adv expenditures"></span><span class="adv clicks"></span><span class="adv shows"></span><span class="adv ctr"></span></span>');
        _context_block_stats.children('.stats-info').children('.adv.expenditures').html("<i>0</i> руб.");
        _context_block_stats.children('.stats-info').children('.adv.ctr').html("<i>0%</i>");
        _context_block_stats.children('.stats-info').children('.adv.shows').html("<i>0</i>");
        _context_block_stats.children('.stats-info').children('.adv.clicks').html("<i>0</i>");
        ...
        _context_block_form.html(_context_block_form.html()+'<i class="save-adv-form continue">Сохранить</i><button class="save-adv-form green">Сохранить и закрыть</button><button class="undo-adv-form grey">Отмена</button>'+'<span class="form-result-box" style="display:none"></span>');
    ...
    //копирование объявления (кнопка 'Копировать')
    jQuery('.central-column').on('click','.copy',function(){
        var _id = jQuery(this).attr("data-id");
        var _url = jQuery(this).attr("data-url");
        if(confirm('Вы уверены, что хотите скопировать контекстное объявление?'))
        jQuery.ajax({
            type: "POST", async: true,
            dataType: 'json', cache: false,
            url: _url,
            ...
            success: function(msg){
                if(msg.ok){
                    jQuery('#context-block-' + _id).before(jQuery("<div id='context-block-" + msg.id + "' class='context-adv' style='display:block;'></div>"));
                    var _new_block = jQuery('#context-block-' + msg.id);
                    _new_block.append(jQuery('#context-block-' + _id).html());
                    
                    //делаем архивным новый блок
                    _new_block.find('.left-block').removeClass('active').addClass('unactive');
                    
                    //заменяем в новом блоке номер, название, статистику, data-url действий
                    _new_block.find('.adv.id').html("#" + msg.id).siblings('.adv.title').html(msg.title);
                    _new_block.find('.stats-info').children().each(function(){
                        jQuery(this).children('i').html("0");
                    });
                    _new_block.children('.object-actions').children('span').each(function(){
                        if(jQuery(this).attr('data-id') != undefined) jQuery(this).attr('data-id',msg.id);
                        if(jQuery(this).attr('data-url') != undefined) jQuery(this).attr('data-url',jQuery(this).attr('data-url').replace(/\d+\/$/,msg.id + '/'));
                    });
                    var _new_form = _new_block.find('#adv-edit-' + _id);
                    _new_form.attr('id','adv-edit-' + msg.id).attr('action',_new_form.attr('action').replace(/\d+\/$/,msg.id + '/'));
                }
                else alert('ошибка копирования объявления');
            },
            error: function(XMLHttpRequest, textStatus, errorThrown){
                return false;
            },
    ...
    jQuery(document).on('click','.delete:not(.outer)',function(){
        var _id = jQuery(this).attr("data-id");
        var _url = jQuery(this).attr("data-url");
        if(confirm('Вы уверены, что хотите удалить контекстное объявление?'))
        jQuery.ajax({
            ...
            success: function(msg){
                if(msg.ok){
                    jQuery('#context-block-'+_id).fadeOut().find('.undo').click();
                }
                else alert('ошибка удаления объявления');
            },
            error: function(XMLHttpRequest, textStatus, errorThrown){
    ...
    jQuery('.add-item').children('.button').on("click",function(){
        ...
        //отвязываем события формы редактирования - очень важно, иначе при каждом новом открытии формы будет дополнительная обработка
        jQuery(document).off('click','.geodata-button.context-tags-apply');
        jQuery(document).off('click','.object-types-picker');
        jQuery(document).off("click",'.delete-image');
        jQuery(document).off("click",'.tg-item');
    ...
    jQuery(document).on("click",'span.edit',function(){
        ...
        if(!jQuery(this).parent().parent().children('.context-adv-edit').hasClass("active")){
            ...
            jQuery(this).parent().parent().children('.left-block-box').hide();
        }
        else{
            //скрываем нашу
    ...
    jQuery(document).on('click','span.undo',function(){
        ...
        if(jQuery(this).hasClass('creation')){
            ...
            //обновляем картинку в списке
            if(jQuery('.public-image').length > 0){
                var _item_id = jQuery(this).attr('data-id');
                if(jQuery('.public-image').children('img').attr('src').length > 0){
                    if(jQuery('#context-block-' + _item_id + ' .left-block-box').find('img').length > 0)
                        jQuery('#context-block-' + _item_id + ' .left-block-box').find('img').attr('src',jQuery('.public-image').children('img').attr('src'));
                    else{
                        jQuery('#context-block-' + _item_id + ' .left-block-box').children('.img-preview').append(jQuery('<div class="campaign-img-box"><div class="campaign-img-frame"><img src=""></div></div>'));
                        jQuery('#context-block-' + _item_id + ' .left-block-box').find('img').attr('src',jQuery('.public-image').children('img').attr('src'));
                    }
                }
            ...
            jQuery(this).parents('.context-adv-edit').parent().children('.left-block-box').show();
    ...
    //дублируем кнопку "Отмена редактирования"
    jQuery(document).on('click','.undo-adv-form.grey',function(){
        jQuery(this).parent().find('.object-actions').children('.undo').click();
    });
            ...
            success: function(msg){
                if(msg.saved){
                    ...
                    if(msg.id){
                        ...
                        jQuery('.context-adv.new').removeClass('edit').removeClass('active');
                    ...
                    if(msg.active == "2" && jQuery('#context-block-'+_item_id).hasClass('active')) jQuery('#context-block-'+_item_id).children('.left-block-box').children('.left-block').removeClass('active').addClass('unactive');
                    if(msg.active == "1" && !jQuery('#context-block-'+_item_id).hasClass('active')) jQuery('#context-block-'+_item_id).children('.left-block-box').children('.left-block').removeClass('unactive').addClass('active');
                    ...
                    if(msg.adv_info!==undefined>0){
                        ...
                        if(msg.adv_info.tags_info!==undefined){
                            jQuery('#context-block-'+_item_id).children('.left-block-box').children('.left-block').children('.targeting-panel').children('.targeting-info').children('.square-info').children('.object-types').html("<b>Тип объекта: </b><i>"+msg.adv_info.tags_info+"</i>");
                        }
                        //тип сделки и недвижимости
                        if(msg.adv_info.deal_text!==undefined){
                            jQuery('#context-block-'+_item_id).children('.left-block-box').children('.left-block').children('.targeting-panel').children('.targeting-estate').html(msg.adv_info.deal_text);
                        }
                        //данные по картинке
                        if(msg.adv_info.photo_info!==undefined){
                            jQuery('#context-block-'+_item_id).children('.left-block-box').children('.left-block').children('.targeting-panel').children('.image-info').html(msg.adv_info.photo_info);
                        }
                        //название объявления
                        if(msg.adv_info.title!==undefined){
                            jQuery('#context-block-'+_item_id).children('.left-block-box').children('.left-block').children('.info-panel').children('.adv.title').html(msg.adv_info.title);
                        }
                    }
                    if(typeof show_success != 'undefined') clearTimeout(show_success);
                    ...
                    if(msg.moderation){
                        ...
                        jQuery('#context-block-'+_item_id).children('.left-block-box').children('.left-block').addClass('moderation');
                        jQuery('#context-block-'+_item_id).children('.object-actions').addClass('moderation');
                    }
                    else{
                        if(msg.archivation){
                            jQuery('#context-block-'+_item_id).children('.left-block-box').children('.left-block').addClass('unactive');
                    ...
                    //по необходимости, обновляем фото в блоке списка
                    if(jQuery('.public-image').length > 0){
                        if(jQuery('.public-image').children('img').attr('src').length > 0){
                            if(jQuery('#context-block-' + _item_id + ' .left-block-box').find('img').length > 0)
                                jQuery('#context-block-' + _item_id + ' .left-block-box').find('img').attr('src',jQuery('.public-image').children('img').attr('src'));
                    ...
                    if(_continue == false){
                        ...
                        //уведомление
                        setTimeout('jQuery(".adv-form-msg").fadeOut(500);clearTimeout();',1000);
                        // щелкаем по кнопке "Отмена", чтобы свернуть форму
                        //alert('!@#' + _item_id + '#@!' + jQuery("#context-block-" + _item_id).find(".undo").length);
                        //if(_item_id.length > 0) _new_item = false;
                        if(_item_id != undefined) _new_item = false;
                        if(_new_item == false){
                            jQuery("#context-block-" + _item_id).find(".undo").click();
                        }else{
                            //$("script[src='/js/estate_search.js']").remove()
                            jQuery('#districts-svg > polygon, #district-areas-svg  > polygon').unbind('click');
                            //_estate_search_included = undefined;
                            //alert('js removed');
                        }
                        //if(_new_item == false) setTimeout('jQuery("#context-block-"' + _item_id + ').find(".undo").click();clearTimeout();',2000);
                    }
                    else{
                        show_success = setTimeout('jQuery(".adv-form-msg").fadeOut(700);clearTimeout();', 4000);
                        ...
                        //скроллим до начала формы (поправка на 36 из-за верхней панели), чтобы было видно уведомление, что все хорошо
                        _form.children('.adv-form-msg').show();
    ...
    //сохраняем данные из формы редактирования объявления и закрываем форму
    jQuery(document).on('click','.save-adv-form',function(){
        //щелкаем по "Сохранить"
        jQuery(this).parent().children('.save-andcont-adv-form').click();
        jQuery(this).parent();
    });
