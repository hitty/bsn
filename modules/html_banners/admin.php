...
switch($action){
	...
	case 'archive':
		...
		if($id>0){
			$res = $db->querys("UPDATE ".$sys_tables['banners']." SET `published` = ? WHERE id=?", $status, $id) or die($db->error);
    ...
    case 'setStatus':
        ...
        if($id>0){
            $res = $db->querys("UPDATE ".$sys_tables['banners']." SET `enabled` = ? WHERE id=?", $status, $id);
