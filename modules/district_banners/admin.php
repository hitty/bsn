...
switch($action){
	...
	case 'archive':
		...
		if($id>0){
			$res = $db->query("UPDATE ".$sys_tables['district_banners']." SET `published` = ? WHERE id=?", $status, $id) or die($db->error);
    ...
    case 'setStatus':
        ...
        if($id>0){
            $res = $db->query("UPDATE ".$sys_tables['district_banners']." SET `enabled` = ? WHERE id=?", $status, $id);
