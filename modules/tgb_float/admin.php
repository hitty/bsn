...
switch($action){
    ...
    case 'phones':
        ...
        switch(true){
            case $ajax_mode:
                ...
                $db->querys("UPDATE ".$sys_tables['tgb_float_phones']." SET status = ? WHERE id = ?",$status,$id);
	...
	case 'archive':
		...
		if($id>0){
			$res = $db->querys("UPDATE ".$sys_tables['tgb_float']." SET `published` = ? WHERE id=?", $status, $id) or die($db->error);
    ...
    case 'setStatus':
        ...
        if($id>0){
            $res = $db->querys("UPDATE ".$sys_tables['tgb_float']." SET `enabled` = ? WHERE id=?", $status, $id);
