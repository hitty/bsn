<?php
$GLOBALS['js_set'][] = '/modules/tgb_overlay/ajax_actions.js';
...
// добавление title
$this_page->manageMetadata(array('title'=>'Overlay баннер'));
...
switch($action){
    ...
    case 'phones':
        ...
        switch(true){
            ...
            default:
                ...
                $paginator = new Paginator($sys_tables['tgb_overlay'], 30);
                ...
                // ссылка пагинатора
                $paginator->link_prefix = '/admin/advert_objects/tgb_overlay/phones' // модуль
                ...
                $sql = "SELECT ".$sys_tables['tgb_overlay_phones'].".*,
                               ".$sys_tables['tgb_overlay'].".title
                        FROM ".$sys_tables['tgb_overlay_phones']."
                        LEFT JOIN ".$sys_tables['tgb_overlay']." ON ".$sys_tables['tgb_overlay_phones'].".id_parent = ".$sys_tables['tgb_overlay'].".id
                        ";
    ...
    case 'stats':
		...
		$GLOBALS['js_set'][] = '/modules/tgb_overlay/datepick_actions.js';
		//получение данных по объекту из базы
		$info = $db->fetch("SELECT 
								`id`,
								`title`,
								IF(img_link='',
									  CONCAT_WS('/','".Config::$values['img_folders']['tgb_overlay']."',`img_src`),
								`img_link`) as photo
							FROM ".$sys_tables['tgb_overlay']."
							WHERE `id` = ?",$id);
		...
		if(!empty($post_parameters['submit'])){
			...
			$stats = $db->fetchall("
					SELECT 
                        ...
                        IFNULL(b.click_amount,0) as click_open_amount, 
                        IFNULL(c.click_amount,0) as click_link_amount, 
                        IFNULL(d.click_amount,0) as click_phone_amount, 
                        a.date 
                    FROM 
					(
					  SELECT 
					  ...
					  FROM ".$sys_tables['tgb_overlay_stats_full_shows']."
					  WHERE
                    ...
                    (
                      SELECT 
                          ...
                          DATE_FORMAT(`date`,'%d.%m.%Y') as date
                      FROM ".$sys_tables['tgb_overlay_stats_full_clicks']."
                      WHERE
                          `date` >= STR_TO_DATE('".$date_start."', '%d.%m.%Y') AND 
					...
					(
                     ...
                     SELECT 
                        ...
                        IFNULL(bb.click_amount,0) as click_open_amount, 
                        IFNULL(cc.click_amount,0) as click_link_amount, 
                        IFNULL(dd.click_amount,0) as click_phone_amount, 
                        'сегодня' as date 
                    FROM 
                    (
                      SELECT 
                         ...
                         'сегодня' as date
                      FROM ".$sys_tables['tgb_overlay_stats_day_shows']."
                      WHERE `id_parent` = ".$id."
                    ) aa
                    LEFT JOIN 
                    (
                      SELECT 
                          ...
                          'сегодня' as date
                      FROM ".$sys_tables['tgb_overlay_stats_day_clicks']."
		...
		break;
	/****************************\
    |*  Работа с баннерами Overlay баннер  *|
    \****************************/		
	case 'add':
	case 'edit':
		...
		if($action=='add'){
			...
			$info = $db->prepareNewRecord($sys_tables['tgb_overlay']);
		} else {
		...
		Response::SetString('img_folder', Config::$values['img_folders']['tgb_overlay']); // папка для Overlay баннер
		// если была отправка формы - начинаем обработку
		if(!empty($post_parameters['submit'])){
			Response::SetBoolean('form_submit', true); // признак того, что форма была обработана
			// проверка значений из формы
			$errors = Validate::validateParams($post_parameters,$mapping['banners']);
			// замена фотографий Overlay баннер
			if(!empty($_FILES)){
				foreach ($_FILES as $fname => $data){
					if ($data['error']==0) {
                        $size = getimagesize($data['tmp_name']);
                        if($size[0]!=1180 && $size[1]!=90) $mapping['banners']['img_src']['error'] = 'Размер файла должен быть 1180x90px. Размер вашего файла'.$size[0].'x'.$size[1].'px';
                        else{
                                
                            $_folder = Host::$root_path.'/'.Config::$values['img_folders']['tgb_overlay'].'/'; // папка для файлов  Overlay баннер
						    $_temp_folder = Host::$root_path.'/img/uploads/'; // папка для файлов  Overlay баннер
			...
			if(empty($errors)) {
				...
				if($action=='edit'){
					...
					$res = $db->updateFromArray($sys_tables['tgb_overlay'], $info, 'id') or die($db->error);
				} else {
					//дата дообавления объекта
					$res = $db->insertFromArray($sys_tables['tgb_overlay'], $info, 'id');
					if(!empty($res)){
						$new_id = $db->insert_id;
						// редирект на редактирование свеженькой страницы
						if(!empty($res)) {
							header('Location: '.Host::getWebPath('/admin/advert_objects/tgb_overlay/edit/'.$new_id.'/'));
							exit(0);
						}
					}
				}
	...
	default:
		...
		//кол-во эл-ов в каждом блоке размещения
		 $sql = "
			SELECT  
				(SELECT IFNULL(COUNT(*),0) FROM ".$sys_tables['tgb_overlay']." WHERE `published` !=3 ) AS alls,
				(SELECT IFNULL(COUNT(*),0) FROM ".$sys_tables['tgb_overlay']." WHERE `published` = 1 and enabled = 1) AS active,
				(SELECT IFNULL(COUNT(*),0) FROM ".$sys_tables['tgb_overlay']." WHERE `published` = 3) AS archive
			FROM dual";
		...
		// создаем пагинатор для списка
		$paginator = new Paginator($sys_tables['tgb_overlay'], 30, $condition);
		// get-параметры для ссылок пагинатора
		$get_in_paginator = array();
		foreach($get_parameters as $gk=>$gv){
			if($gk!='page') $get_in_paginator[] = $gk.'='.$gv;
		...
		// ссылка пагинатора
		$paginator->link_prefix = '/admin/advert_objects/tgb_overlay'                // модуль
								  ."/?"                                       // конечный слеш и начало GET-строки
								  .implode('&',$get_in_paginator)             // GET-строка
								  .(empty($get_in_paginator)?"":'&')."page="; // параметр для номера страницы
		if($paginator->pages_count>0 && $paginator->pages_count<$page){
		...
		$sql = "SELECT 
                        ban.*, 
						IF(ban.img_link='',
							  CONCAT_WS('/','".Config::$values['img_folders']['tgb_overlay']."',`ban`.`img_src`),
				...
				FROM ".$sys_tables['tgb_overlay']." ban
		        LEFT JOIN (SELECT COUNT(*) as cnt_day, id_parent FROM ".$sys_tables['tgb_overlay_stats_day_shows']." GROUP BY id_parent) a ON a.id_parent = ban.id	
		        LEFT JOIN (SELECT SUM(amount) as cnt_full, id_parent FROM ".$sys_tables['tgb_overlay_stats_full_shows']." GROUP BY id_parent) b ON b.id_parent = ban.id	
		        LEFT JOIN (SELECT COUNT(*) as cnt_click_day, id_parent FROM ".$sys_tables['tgb_overlay_stats_day_clicks']." GROUP BY id_parent) c ON c.id_parent = ban.id		
                LEFT JOIN (SELECT SUM(amount) as cnt_click_full, id_parent FROM ".$sys_tables['tgb_overlay_stats_full_clicks']." GROUP BY id_parent) d ON d.id_parent = ban.id       
		        ";		
		if(!empty($condition)) $sql .= " WHERE ".$condition;
		$sql .= " ORDER BY ban.id";
		$sql .= " LIMIT ".$paginator->getLimitString($page); 
