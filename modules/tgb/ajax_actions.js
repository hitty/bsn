jQuery(document).ready(function(){
    ...
    managePopunder();
    jQuery('#checkbox_credit_clicks').on('click', function(){
        managePopunder();    
    })

    
});
function managePopunder(){
    var _checked = jQuery('#checkbox_credit_clicks').is(':checked')
    if(_checked == true) jQuery('#p_field_only_popunder_clicks').show();
    else {
        var _radios = jQuery('input:radio[name=only_popunder_clicks]');
        jQuery('input:radio[name=only_popunder_clicks]').filter('[value=3]').prop('checked', true);
        jQuery('#p_field_only_popunder_clicks').hide();
    }
}
function manageUtm(){
