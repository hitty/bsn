jQuery(document).ready(function(){
        ...
        jQuery.ajax({
            ...
            success: function(msg){
                if(typeof(msg)=='object' && msg.ok) {
                    jQuery('.list_table').children('table').children('tbody').append("<tr><td><a title='"+msg.data.title+"' href="+msg.data.url+">"+msg.data.url+"</a></td> <td>"+msg.data.title+"</td> <td>"+msg.data.date_in+"</td> <td></td> <td class=\"small_icons ac\"><a href=\"/admin/pages/not_indexed/google/del/\" title=\"Удалить\" target=\"_blank\"><span class=\"ico_del\">Удалить</span></a></td></tr>");
    ...
    jQuery('.page-index-add .button.check').on('click',function(){
        ...
        var _page_id = jQuery('#autocomplete_value').val();
        var _url = "/admin/pages/not_indexed/check/";
        jQuery.ajax({
            type: "POST", dataType: 'json',
            async: true, cache: false,
            url: _url,
    ...
    function getData(){  // получение информации с сервера
        ...
        var _bot_alias = window.location.href.split('/')[6];
        var _url = '/admin/pages/not_indexed/'+_bot_alias+'/';
        jQuery.ajax({
