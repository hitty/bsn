...
$list = $housing_estates->Search($sys_tables['housing_estates'].'.published = 1', 10000, 0);
    ...
    if(!empty($photos_list)){
        foreach($photos_list as $p => $photo){
            //if(file_exists(ROOT_PATH."/".Config::Get('img_folders/live')."/big/".$photo['subfolder']."/".$photo['name']))
                $array[$k]['photo_'.($p+1)] = "https://www.bsn.ru/".Config::Get('img_folders/live')."/big/".$photo['subfolder']."/".$photo['name'];
        }
    }
}
