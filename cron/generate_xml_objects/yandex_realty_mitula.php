...
define('__XMLPATH__',ROOT_PATH.'/yandex_realty_mitula.xml');
...
$where =   "
            ".$sys_tables['live'].".id_user != 1454  AND
...
$where =   "
            ".$sys_tables['build'].".id_user != 1454  AND
...
foreach($deal_types as $val => $deal_type){
    ...
    $where .= " AND ".$sys_tables['country'].".`published` = 1
                AND ".$sys_tables['country'].".id_user != 1454
...
// параметры письма
$mailer->Subject = iconv('UTF-8', $mailer->CharSet, 'Генерация фида в Митула');
$mailer->Body = $html;
...
$mailer->FromName = iconv('UTF-8', $mailer->CharSet,'Генерация фида в Митула');
// попытка отправить
$mailer->Send();        
