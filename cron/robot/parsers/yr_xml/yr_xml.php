...
$agency_xml_info = $db->fetch("SELECT ".$sys_tables['agencies'].".id,
                               ...
                               WHERE ".$sys_tables['users'].".id = 48351");
//WHERE ".$sys_tables['agencies'].".xml_time = ".date('H')." AND (xml_day=0 OR xml_day=".date('N').") AND xml_status=1 AND xml_type=6");
echo $db->last_query;
...
if(!empty($log['download'])){
    ...
    if(!empty($agency_xml_info['id']) && !empty($xml_size)) $db->querys("UPDATE ".$sys_tables['agencies']." SET xml_file_processing = 0, xml_last_size = ".$xml_size." WHERE id = ".$agency_xml_info['id']);
}
?>
