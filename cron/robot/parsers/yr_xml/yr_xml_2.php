...
$agency_xml_info = $db->fetch("SELECT ".$sys_tables['agencies'].".id,
                               ...
                               WHERE ".$sys_tables['users'].".id = 48401");
if($agency_xml_info) $log['download'][] = downloadXmlFile("yr",$agency_xml_info['xml_alias'],$agency_xml_info['xml_link'],$agency_xml_info['id_user'],false);

$rent_titles = array(1=>'аренда', 2=>'продажа'); //типы сделок

//папка с xml файлами 
$dir = ROOT_PATH."/cron/robot/files/yr_xml/";
        ...
        else
        {
            ...
            elseif(!empty($agency['id_tarif'])) $mail_text .= "Объекты агентства ".$agency['title']." не выгрузились, потому что для агентства установлен тариф ".$agency['tarif_title'];
                ...
                if($update_flag){
                    ...
                    $db->querys("UPDATE ".$sys_tables['live']." SET `published` = '2', `date_change` = NOW() WHERE id_user = ? AND info_source=? AND `published` = 1",$id_user,8);
                    $db->querys("UPDATE ".$sys_tables['build']." SET `published` = '2', `date_change` = NOW() WHERE id_user = ? AND info_source=? AND `published` = 1",$id_user,8);
                    $db->querys("UPDATE ".$sys_tables['country']." SET `published` = '2', `date_change` = NOW() WHERE id_user = ? AND info_source=? AND `published` = 1",$id_user,8);
                    $db->querys("UPDATE ".$sys_tables['commercial']." SET `published` = '2', `date_change` = NOW() WHERE id_user = ? AND info_source=? AND `published` = 1",$id_user,8);
                    $update_flag = false;
                }
                $xml_values = array(); 
                ...
                if($xml_str===FALSE || empty($xml_str['realty-feed']['offer'][0]))  $errors_log['fatal'] = 'Файл '.$dir.$filename.' не может быть обработан, т.к. имеет невалидные теги'; 
                else {
                    foreach ($xml_str['realty-feed']['offer'] as $object) if(empty($object['internal-id'])) $xml_values[] =  $object;
                    ...
                    foreach($xml_values as $key=>$values){
                        ...
                        //записываем internal-id
                        $values['internal-id'] = $internal_ids[$offer_key];
                        ...
                        $check_limit = ($robot->estate_type.$deal_type == 'live_rent' && $counter[$robot->estate_type.$deal_type] < $agency[$robot->estate_type.$deal_type.'_objects']) || ($robot->estate_type.$deal_type != 'live_rent' && ($agency['id_object_packets']==1 || $agency['id_object_packets']==7 || $counter[$robot->estate_type.$deal_type] < $agency[$robot->estate_type.$deal_type.'_objects'])); 
                        if(!empty($fields)) {
                            //сумма всех итераций
                            ++$counter['total'];
...
if($mail_text!=''){
    ...
    // параметры письма
    $mailer->Subject = iconv('UTF-8', $mailer->CharSet, 'Обработка формата Yandex XML. '.date('Y-m-d H:i:s'));
    $mailer->Body = $html;
    ...
    $mailer->FromName = iconv('UTF-8', $mailer->CharSet,'Парсинг Yandex XML файла');
    // попытка отправить
    $mailer->Send();        
}
...
if(!empty($log['download'])){
    ...
    // параметры письма
    $mailer->Subject = iconv('UTF-8', $mailer->CharSet, 'eip_xml.php : Проверка выгрузки файлов Yandex XML. '.date('Y-m-d H:i:s'));
    $mailer->Body = $html;
    $mailer->AltBody = strip_tags($html);
    $mailer->IsHTML(true);
