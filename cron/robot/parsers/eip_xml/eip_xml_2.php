...
while($filename = readdir($dh))
{
    if($filename!='.' && $filename!='..')
    {
        ...
        else
        {
            ...
            if(empty($agency)) $mail_text.="Ошибка авторизации"; //агентство не найдено
                ...
                if($update_flag){
                    ...
                    $db->query("UPDATE ".$sys_tables['live']." SET `published` = '2', `date_change` = NOW() WHERE id_user = ? AND info_source=? AND `published` = 1",$id_user,3);
                    $db->query("UPDATE ".$sys_tables['build']." SET `published` = '2', `date_change` = NOW() WHERE id_user = ? AND info_source=? AND `published` = 1",$id_user,3);
                    $db->query("UPDATE ".$sys_tables['country']." SET `published` = '2', `date_change` = NOW() WHERE id_user = ? AND info_source=? AND `published` = 1",$id_user,3);
                    $db->query("UPDATE ".$sys_tables['commercial']." SET `published` = '2', `date_change` = NOW() WHERE id_user = ? AND info_source=? AND `published` = 1",$id_user,3);
                    $update_flag = false;
                }
                $xml_values = array(); 
        ...
        /*
        $counter_real['live_rent'] = $counter['live_rent'];
        $counter_real['live_sell'] = $counter['live_sell'];
        $counter_real['commercial_rent'] = $counter['commercial_rent'];
        $counter_real['commercial_sell'] = $counter['commercial_sell'];
        $counter_real['country_rent'] = $counter['country_rent'];
        $counter_real['country_sell'] = $counter['country_sell'];        
        $counter_real['build'] = $counter['build'];
        */
        
        //для Адвекс-новостроек, удаляем дубли
        if($id_user == 3991){
            $duplicates = array();
