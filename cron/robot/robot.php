...
foreach($estates as $estate_type=>$prefix){
    //все новые объекты
    $list = $db->fetchall("SELECT `id` FROM ".$sys_tables[$estate_type.'_'.$prefix]." ORDER BY date_in DESC");
    if($list){
        foreach($list as $key=>$val){
            $moderate = new Moderation($estate_type,$val['id']);
            $moderate->checkObject();
        }
    }
}

 Простановка кол-ва вариантов для агентств, если выбран тарифный план */
//updateAgenciesByPackets();
