...
$db->querys("INSERT INTO ".$sys_tables['news_parsing_stats']." (run_type,sources_ids) VALUES (".$run_type.",'".implode(',',array_keys($sources_list))."')");
...
foreach($sources_list as $source_key=>$source){
    ...
    $db->querys("DELETE FROM ".$sys_tables['news_parsing_stats']." WHERE end_datetime LIKE '%000%'");
...
$db->querys("UPDATE ".$sys_tables['news_parsing_stats']." SET end_datetime = NOW(), articles_readed = ?, articles_added = ? WHERE id = ?",$links_total,$links_total_added,$this_parsing_id);
