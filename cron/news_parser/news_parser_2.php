...
$run_type = !empty($_SERVER['argv']) && !empty($_SERVER['argv'][1]) ? $_SERVER['argv'][1] : false;
    ...
    require_once('includes/class.photos.php');        //для фоток
...
//список источников
$sources_list = $db->fetchall("SELECT * FROM ".$sys_tables['news_sources']." WHERE id = 1",'id');
...
$links_total_added = 0;
error_reporting(E_ALL);
$SERVER['SERVER_ADDR'] = "98.138.253.109";
$SERVER['REMOTE_ADDR'] = "98.138.253.109";
$SERVER['HTTP_USER_AGENT'] = "Mozilla/5.0 (compatible; Yahoo! Slurp; http://help.yahoo.com/help/us/ysearch/slurp)";
$SERVER['HTTP_HOST'] = "help.yahoo.com";

$curl = curl_init();
$url = $sources_list[1]['url'];
...
curl_setopt($ch, CURLOPT_STDERR, "curlerrors.log");
$header = explode("\n", curl_exec($curl));
echo "error: ".curl_error($curl)."!\r\n";
echo "error: ".curl_errno($curl)."!\r\n";
echo "info: ".json_encode(curl_getinfo($curl))."\r\n";
curl_close($curl);
die();


//echo gethostbyname("http://www.gazeta.bn.ru");die();
//echo gethostbyaddr("188.93.21.160");die();

echo $sources_list[1]['url']." resp = ".get_http_response_code("bn.vader.bn.ru")."!@#";die();
die();
foreach($sources_list as $source_key=>$source){
    ...
    echo "response = ".$source." : " .get_http_response_code($source['url']);
    ...
    foreach($links as $k=>$link){
        ...
        if(check_article($article_head)){
            $exists = $db->fetch("SELECT id FROM ".$sys_tables['news_articles_obtained']." WHERE url = ?",$article_url);
            if(empty($exists)){
                $db->insertFromArray($sys_tables['news_articles_obtained'],$article_to_insert);
                $new_id = $db->insert_id;
                ++$counter;
                ...
                if(!empty($images)){
                    foreach($images as $image_key=>$image_link){
                        ...
                        //Photos::Add('news_articles_obtained',$new_id,NULL,$image_link,false,300,200,true);
                    }
                }
            }
        }
