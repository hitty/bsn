...
if(!empty($debug)) $date_where = '';
    ...
    $main_news = $news->getList( 1, 0, false, false, 'newsletter_feed = 1 AND newsletter_title=1' );
    $newsletter_title = !empty($main_news) ? $main_news[0]['title'] : "Новости рынка недвижимости от БСН за ".date("d.m.Y");
    if( !empty( $main_news[0] ) ) {
        $main_news = $main_news[0];
        Response::SetString( 'mailer_title', $main_news['title'] );
        Response::SetArray( 'main_news', $main_news );
        //заглавная фотка
        $dest = $root . '/img/uploads/mailers/' . $main_news['photo'];
        $watermark_src = '/img/layout/mailer/black_bg.png';
        Photos::imageResize( 
            $root . '/img/uploads/big/' . $main_news['subfolder'] . '/' . $main_news['photo'], 
            $dest, 
            580, 
            250, 
            'cut',  
    ...
    $news_list = $news->getList(8, 0, false, false, $sys_tables['news'] . '.id != ' . $main_news['id'] . ' AND newsletter_feed = 1 '.$date_where, $sys_tables['news'].".status = 2 DESC, ".$sys_tables['news'].".views_count DESC, ".$sys_tables['news_categories'].".position, ".$sys_tables['news'].".id DESC");
    if(!empty($news_list)) Response::SetArray('news_list',$news_list);

    //получение списка новостей БСН.ТВ
    $bsn_tv = new Content('bsntv');
    $bsn_tv_news_list = $bsn_tv->getList(false, false, false, false, 'newsletter_feed = 1 '.$date_where, $sys_tables['bsntv'].".status = 2 DESC, ".$sys_tables['bsntv'].".views_count DESC, ".$sys_tables['bsntv_categories'].".position, ".$sys_tables['bsntv'].".id DESC");
    if(!empty($bsn_tv_news_list)) {
        if( DEBUG_MODE ) $bsn_tv_news_list = array_slice( $bsn_tv_news_list, 0 , 3 );
    ...
    if( !empty( $doverie_news_list ) ) {
        if( DEBUG_MODE ) $doverie_news_list = array_slice( $doverie_news_list, 0 , 3 );
...
//мнения экспертов
    ...
    if( !empty( $articles_list ) ) $list = array_merge( $list, $articles_list );
...
Response::SetString( 'mailer_type', 'Новостной дайджест' );

Response::SetArray('env',array('url'=>Host::GetWebPath('/'),'host'=>Host::$host));

$email_list = $db->fetchall("SELECT DISTINCT s.email, s.id FROM ( 
                                (SELECT email, id FROM ".$sys_tables['users']." WHERE YEAR(last_enter) > 2015 AND email!='' AND active_email = 1 AND subscribe_news = 1 ) 
                                UNION
