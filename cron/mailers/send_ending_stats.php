...
$users_objects = array();
$objects_stats = array_fill_keys(array_keys($estate_types),array());
foreach($estate_types as $estate_type=>$days){
    ...
    $active_objects = $db->fetchall("SELECT ".$sys_tables[$estate_type].".id,
                                     ...
                                     WHERE ( (".$sys_tables[$estate_type].".status>2 AND ".$sys_tables[$estate_type].".status_date_end < CURDATE()) OR
                                             (`date_change` < (CURDATE() - INTERVAL ".$days." day))
                                           ) AND
                                           ".$sys_tables['users'].".id_agency = 0 AND
                                           published = 1", 'id' );
    if(empty($active_objects)) continue;
    $users_ids = array();
    foreach($active_objects as $key=>$object){
        if(empty($users_objects[$object['id_user']])) $users_objects[$object['id_user']]['objects_info'] = array_fill_keys( array_keys($estate_types), array() );
    ...
    $min_stats_date = $db->fetch("SELECT IF(DATE_FORMAT(MIN(date_change),'%Y-%m-%d') > DATE_SUB(NOW(),INTERVAL 30 DAY),
                                  ...
                                  WHERE ( (".$sys_tables[$estate_type].".status>2 AND ".$sys_tables[$estate_type].".status_date_end = CURDATE()) OR
                                        ...
                                        id_user IN (".implode(',',$users_ids).") AND 
                                        published = 1")['stats_from_date'];
        ...
        if(!empty($user_info['stats_shows']) || !empty($user_info['stats_search']) || !empty($user_info['stats_phones'])){
            ...
            foreach($user_info['objects_info'] as $estate_type => $objects_info){
                foreach($objects_info as $key => $object_id){
                    ...
                    $fileName = ROOT_PATH."/img/uploads/mail_charts/".$object_id."_ending_".$stats_type."_".date('Y-m-d').".png";
                    $bottom_margin = 40;
                    ...
                    $eml_tpl = new Template('mail.user.ending_stats.html', 'cron/mailers/');
                    
                    $object_info['estate_type'] = $estate_type;
                    $object_info['object_id'] = $object_id;
                    //$object_info['link'] = "https://www.bsn.ru/".$estate_type."/".($object_info['rent'] == 1 ? 'rrent' : 'sell')."/".$object_id."/";
                    //$object_info['pay_link'] = "https://www.bsn.ru/members/edit/".$estate_type."/".($object_info['rent'] == 1 ? 'rent' : 'sell')."/".$object_id."/?step=3";
                    ...
                    Response::SetString('chart_src',"//st1.bsn.ru/img/uploads/mail_charts/".$object_id."_ending_".$stats_type."_".date('Y-m-d').".png?v=". rand(0,10000));
