...
$list=$db->fetchall('SELECT *,LEFT (`up_banner`,2) as `subfolder_up`,LEFT (`down_banner`,2) as `subfolder_down` FROM '.$sys_tables['normalspam'].' WHERE published=1');
if(!empty($list)){

    $list_emails=$db->fetchall('SELECT DISTINCT a.email, a.id FROM (
                                    SELECT DISTINCT email, id FROM '.$sys_tables['subscribed_users'].' WHERE published = 1
                                    UNION
                                    SELECT DISTINCT email, id FROM '.$sys_tables['users'].' WHERE '.$sys_tables['users'].'.subscribe_news=1
    ...
    foreach ($list as $item){
	   if( !empty( $item['subject'] )) {
	    ...
	    $db->insertFromArray(
	        ...
	        array( 'title' => 'Рассылка по подписчикам БСН: ' . $item['subject'] )
	    );
        ...
        $db->querys('UPDATE '.$sys_tables['normalspam'].' SET published=2, begin_datetime = NOW() WHERE id=?',$item['id']);
        // параметры письма
        ...
        foreach($list_emails as $item_email){
            ...
            $mailer->Subject = iconv('UTF-8', $mailer->CharSet . '//IGNORE', $item['subject']);
            $mailer->Body = $html;
        ...
        $db->querys('UPDATE '.$sys_tables['normalspam'].' SET end_datetime = NOW() WHERE id=?',$item['id']);
