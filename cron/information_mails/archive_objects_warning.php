...
$list = $db->fetchall("
        ...
        (
            (
                ...
                WHERE estate_table.published = 2 AND estate_table.info_source = 1 AND estate_table.date_change = CURDATE() - INTERVAL 30 DAY AND ".$sys_tables['users'].".email!='' 
            ) UNION (
                ...
                LEFT JOIN ".$sys_tables['users']."  ON ".$sys_tables['users'].".id = estate_table.id_user
                WHERE estate_table.published = 2 AND estate_table.info_source = 1 AND estate_table.date_change = CURDATE() - INTERVAL 60 DAY AND ".$sys_tables['users'].".email!='' 
            ) UNION (
...
Напоминаем, что следующие ваши объявления <b>перемещены в архив</b>:<br><br>';
        $html .= implode("<br />",$text['text']);
        $html .= '<br><br>
        Продлить данные объявления (или добавить новые) Вы можете в <b>Личном кабинете</b>:
        <a href="https://www.bsn.ru/members/objects/">https://www.bsn.ru/members/objects/</a>
        <br /><br />
