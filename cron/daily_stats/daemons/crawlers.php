...
foreach($crawlers as $key=>$item){
    $res = $res && $this->db->query("INSERT INTO ".$sys_tables['pages_visits_'.$item.'_full']." (`date`,visits_amount,links_shown,old_pages_visits,pages_added) VALUES
    ...
    $res = $res && $this->db->query("UPDATE ".$sys_tables['pages_not_indexed_'.$item]." SET shown_today = 0");
    $res = $res && $this->db->query("TRUNCATE ".$sys_tables['pages_visits_'.$item.'_day']);
    
    //раз в месяц чистим переходы с поиска и показанные страницы
    if(date('j') == 1){
        $this->db->query("UPDATE ".$sys_tables['pages_not_indexed_'.$item]." SET googletm = '0000-00-00 00:00:00' WHERE DATEDIFF(NOW(),googletm)>30");
        $this->db->query("UPDATE ".$sys_tables['pages_not_indexed_'.$item]." SET yandextm = '0000-00-00 00:00:00' WHERE DATEDIFF(NOW(),yandextm)>30");
        $this->db->query("UPDATE ".$sys_tables['pages_not_indexed_'.$item]." SET mailrutm = '0000-00-00 00:00:00' WHERE DATEDIFF(NOW(),mailrutm)>30");
        $this->db->query("UPDATE ".$sys_tables['pages_not_indexed_'.$item]." SET has_shown = 0");
    }
}
//$res = $res && $this->db->query("INSERT INTO ".);
