...
$res = $res && $this->db->querys("UPDATE ".$sys_tables['banners']." SET `enabled`=2, `published`=2 WHERE `date_end` <= CURDATE() and enabled=1");
...
$res = $res && $this->db->querys("UPDATE ".$sys_tables['district_banners']." SET `enabled`=2, `published`=2 WHERE `date_end` <= CURDATE() and enabled=1");
...
$res = $res && $this->db->querys("UPDATE ".$sys_tables['tgb_vertical']." SET `enabled`=2, `published`=2 WHERE `date_end` <= CURDATE() and enabled=1");
...
$res = $res && $this->db->querys("UPDATE ".$sys_tables['tgb_float']." SET `enabled`=2, `published`=2 WHERE `date_end` <= CURDATE() and enabled=1");
...
$res = $res && $this->db->querys("UPDATE ".$sys_tables['credit_calculator']." SET `enabled`=2, `published`=2 WHERE `date_end` <= CURDATE() and enabled=1");
$log['cc_arch'] = "Снятие актуальности с кредитных калькуляторов просрочивших дату показа: ".((!$res)?$this->db->error:"OK")."<br />";
$res = true;

//снятие актуальности с б кредитных калькуляторов просрочивших дату показа
$res = $res && $this->db->querys("UPDATE ".$sys_tables['tgb_overlay']." SET `enabled`=2, `published`=2 WHERE `date_end` <= CURDATE() and enabled=1");
...
$res = $res && $this->db->querys("UPDATE ".$sys_tables['tgb_banners']." SET `credit_time` = '00:00:00'");
...
$res = $res && $this->db->querys("UPDATE ".$sys_tables['tgb_banners']." SET `enabled`=2, `published`=2, `clicks_limit` = 0, `credit_clicks` = 2, clicks_limit_notification = 1 WHERE `date_end` <= CURDATE() and enabled=1");
