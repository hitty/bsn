...
$error_log = ROOT_PATH.'/cron/estate/error.strip_notes.log';
...
foreach($estates as $estate_type){
    $list = $db->fetchall("SELECT * FROM ".$sys_tables[$estate_type]." WHERE published = 1");
...
foreach($estates as $estate_type){
    $list = $db->fetchall("SELECT * FROM ".$sys_tables[$estate_type]." WHERE published = 2");
    foreach($list as $k=>$item){
        $notes = Validate::stripEmail($item['notes']);
        $notes = Validate::stripPhone($notes);
        $db->querys("UPDATE ".$sys_tables[$estate_type]." SET notes = ? WHERE id = ?",$notes,$item['id']);
